...
void setup() 
{
  ...
  // TODO: Setup subcriber for initializing the motors and wait for initial number of rotations
  ...
  for (int i = 0; i < NUMBER_OF_MOTORS; i++)
  {
    ...
    // CAN port 1 should be used for initilization
    if(i < NUMBER_OF_MOTORS_PER_PORT)
    {
      motors[i].initialize(can_port1);
    }
    // CAN port 2 should be used
    else
    {
      motors[i].initialize(can_port2);
    }
  }
