function [F, dF, ddF] = calcAugmentedLagrangian_nonsparse(x, variable_idx, U, P, w, A0, A)
...
Z_x = -blkdiag(M1_P_Inv, M2_P_Inv, inv(M3 + P));
...
for i = 1:n_xi
    ...
    dM1 = [zeros(n_W), dM12;
        dM12', zeros(n_W)];
    dA = -blkdiag(dM1, zeros(n_xi+2)); % +1 for gamma in M2 and +1 for mu in the last inequality
...
dM = blkdiag(zeros(n_W), -z.W);

dAdmu = -blkdiag(dM, zeros(n_xi+1), 1);
...
for i = 1:n_W
    ...
    for k = 1:n_xi
        ddM12 = A{k}*dWdWvec;
        ddM1 = [zeros(n_W), ddM12;
            ddM12', zeros(n_W)];
        ddA = -blkdiag(ddM1, zeros(n_xi+2)); % +1 for gamma in M2 and +1 for mu in the last inequality
        ddPhi = P^2*Z_x*(dAdXi{k}*Z_x*dA + dA*Z_x*dAdXi{k} - ddA)*Z_x;
        ddFdxidWvec(idx_vec(i), k) = trace(U*ddPhi);
    end
    %ddF/dmudWvec
    
    ddA = -blkdiag(zeros(n_W), -dWdWvec, zeros(n_xi+2));
...
dA = -blkdiag(zeros(n_W*2 + n_xi), 1, 0);
