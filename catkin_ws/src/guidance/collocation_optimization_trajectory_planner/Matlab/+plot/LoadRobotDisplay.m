function robot_disp = LoadRobotDisplay(robot, varargin)
    
    
    
    
    
    ip = inputParser;
    ip.addParameter('UseExported',true,@(x) isequal(x,true) || isequal(x,false));
    ip.addParameter('ExportPath','',@(x)ischar(x));
    ip.addParameter('SkipExporting',false,@(x) isequal(x,true) || isequal(x,false));
    
    ip.parse(varargin{:});
    
    opts = ip.Results;
    if isempty(opts.ExportPath)
        root_path = utils.get_root_path();
        export_path = fullfile(root_path,'gen','animator');
        opts.ExportPath = export_path;
    else
        export_path = opts.ExportPath;
    end
    if ~exist(export_path,'dir')
        mkdir(export_path);
    end
    addpath(export_path);
    
    f = figure(1000);clf;
    robot_disp = frost.Animator.Display(f, robot, opts);frame  = robot.Joints(6);
    
    
    pelvis = robot.Links(getLinkIndices(robot,'base_link'));    
    offset = pelvis.Offset;    
    torso_bar = frost.Animator.Cylinder(robot_disp.axs, robot, frame, offset, 'TorsoBar', opts);
    robot_disp.addItem(torso_bar);
    
    torso = frost.Animator.LinkSphere(robot_disp.axs, robot, pelvis, 'TorsoCoM', opts);
    torso.radius = 0.1;
    robot_disp.addItem(torso);
    % add feet
    fl_foot = sys.frames.FlFoot(robot);
    fr_foot = sys.frames.FrFoot(robot);
    rl_foot = sys.frames.RlFoot(robot);
    rr_foot = sys.frames.RrFoot(robot);
    
    fl_foot = frost.Animator.LinkSphere(robot_disp.axs, robot, fl_foot, 'FlFoot', opts);
    fr_foot = frost.Animator.LinkSphere(robot_disp.axs, robot, fr_foot, 'FrFoot', opts);
    rl_foot = frost.Animator.LinkSphere(robot_disp.axs, robot, rl_foot, 'RlFoot', opts);
    rr_foot = frost.Animator.LinkSphere(robot_disp.axs, robot, rr_foot, 'RrFoot', opts);
    
    
    
    robot_disp.addItem(fl_foot);
    robot_disp.addItem(fr_foot);
    robot_disp.addItem(rl_foot);
    robot_disp.addItem(rr_foot);    
end