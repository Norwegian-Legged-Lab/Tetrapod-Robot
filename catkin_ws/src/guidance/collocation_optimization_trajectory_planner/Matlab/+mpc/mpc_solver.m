classdef mpc_solver
    ...
    methods
        function obj = mpc_solver(m, I_b, mu, r_body_to_fl_ref, delta_t, horizon, cycle_period, ref_cost, force_cost, stance_info)
            ...
            obj.r_body_to_fl_ref = reshape(r_body_to_fl_ref, [], 1);
            obj.mu = mu;
        ...
        function stance_indices = getStanceIndices(obj, t0)
            ...
            stance_indices.fl.stance = mpc.is_stance('fl', tau_s, obj.stance_info);
            stance_indices.fr.stance = mpc.is_stance('fr', tau_s, obj.stance_info);
            stance_indices.rl.stance = mpc.is_stance('rl', tau_s, obj.stance_info);
            stance_indices.rr.stance = mpc.is_stance('rr', tau_s, obj.stance_info);
        ...
        function sol = get_solution(obj, x, stance_vectors, t0)
            ...
            stance_indices = mpc.get_stance_indices(obj.cycle_period, obj.horizon, obj.delta_t, t0, obj.stance_info);
        ...
        function msg = solToRosMsg(obj, sol)
            ...
            msg.Inputs.Fl = obj.solFootInputToRosFootInput(sol.inputs.fl);
            msg.Inputs.Fr = obj.solFootInputToRosFootInput(sol.inputs.fr);
            msg.Inputs.Rl = obj.solFootInputToRosFootInput(sol.inputs.rl);
            msg.Inputs.Rr = obj.solFootInputToRosFootInput(sol.inputs.rr);
        end
        
        function mpcResponseMsg= solve_ros(obj, mpcRequestMsg)
            x0 = mpcRequestMsg.X0;
            t0 = mpcRequestMsg.T0;
            ...
            for i = 1:length(field_names)
                if mpcRequestMsg.InitStancePos.([field_names{i} 'Set'])
                    init_stance_pos.(lower(field_names{i})) = mpcRequestMsg.InitStancePos.([field_names{i} 'Pos']);
                end
            end
            vel_des = mpcRequestMsg.VelDes;
            yaw_rate_des = mpcRequestMsg.YawRateDes;
            sol = obj.solve(x0, t0, init_stance_pos, vel_des, yaw_rate_des);
            mpcResponseMsg = obj.solToRosMsg(sol);
        end
    end
end
